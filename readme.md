...
2. Reveal by microsite name and traffic how much this severity ought to matter to you as you organize your staff efforts to repair the site. While traffic should certainly play a role in how you manage your site, you should NOT use traffic data in a way that damages your relationship with small target audiences. All site traffic should be judged relative to the size of the target audience for that content.

To start with, these aspirations require a foundational metaphor for your whole ecology - a map - a landscape that you can explore with your eyes. See [David McCandless, 2010](https://www.youtube.com/watch?v=5Zg-C8AAIGg). Then, when any new situation arises, you overlay new reporting, consisting of URL-Problem Description, quickly and easily onto the existing map, by itself or together with other problem reports, so all teams can comprehend the situation they are in and start on the shortest path to positive action.
...
Web site repair does not need to be spotty or haphazard. Nor should an organization's content components be like leaves falling on a lake - highly visiable at first, but gradually sinking out of site and out of mind, and yet, still there. Instead, you should proactively UNDERSTAND and PRIORITIZE work that should be done, based on an ecology-wide view encompassing your MISSION, the problem's severity, your staff availability, etc.


### Screenshot (but see [Gist](http://bl.ocks.org/wendlingd/af1e751e97c5211ff11277c985e5e642) for interactive version)

Rectangle volume shows relative number of HTML pages; some communication packages are small; some are large. Gray shading / zero points means no errors were found. Shade of blue / total count of points show severity of the problem being analyzed. White shading means a microsite was not accessed during the period when traffic data was collected. 

In this version of the map, points are used rather than the count of broken links, so management can increase the severity of a particular problem the organization wants solved before others. When one repair should be completed ASAP, staff will want to focus on that one problem. When more time is available, staff can overlay multiple problem reports so they can make multiple types of repair while microsites are under the editorial microscope. In this display, broken link counts were multiplied by two in order to make broken links more prominent among multiple types of problems are being analyzed at the same time.
...
- Create a database and add a 100-percent content inventory from a web spidering tool or a programmer's tool such as the HTML::Tree module of Perl or other spidering tool.
